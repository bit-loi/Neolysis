...
- Optional pretrained ESM2 embeddings (inference only; no model training required)
- Explicit uncertainty metadata and per-run reproducibility records
- Optional grounded LLM narrative that cannot replace or modify scientific scores
...
### Training-Free Model Path and LLM Boundary

Neolysis can use a pretrained ESM2 representation without training a model from scratch. Set:

```bash
PROTEIN_EMBEDDING_MODE=esm2
PROTEIN_EMBEDDING_MODEL=facebook/esm2_t6_8M_UR50D
```

The model is loaded lazily on the first request. If it cannot be loaded, the API falls back to the lightweight composition embedding and returns `status: fallback` plus a `fallback_reason`; it never silently presents the fallback as ESM2. A protein embedding is a representation, not a validated function classifier, so function and property outputs remain clearly labeled as uncalibrated prioritization signals.

`POST /api/v1/agents/analyze` accepts `narrative_mode: deterministic | llm`. The default is deterministic. With `llm`, the configured model only narrates immutable structured tool output at low temperature. If no API key is configured or generation fails, the deterministic report is returned. The response records `llm_used`, the model name, component versions, timestamp, deterministic seed, and a SHA-256 input fingerprint under `provenance`.

Every prediction exposes an `uncertainty` object. Current values are explicitly marked `uncalibrated`; they are not probabilities of wet-lab success. External dataset benchmarking and assay-specific calibration are still required before stronger claims can be made.
...
The workspace is compatible with Neon CLI linking. A root `.env.local` generated by `neon link` is loaded by the backend automatically; provider PostgreSQL URLs are normalized to SQLAlchemy's async `asyncpg` driver without committing database credentials.
...
- Function prediction is a motif-based scaffold, not a trained or calibrated enzyme classifier; optional ESM2 embeddings do not change that claim.
